...
int main (int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i += 2)
    if (!strcmp (argv[i], "-i") && argc >= i + 1)
      ...
      printf
	("Usage: %s [-n] num_clients [-f] file_name [-i] ip_addr [-p] port\n",
	 argv[0]);
  ...
  if (num_clients <= 0 || num_clients > MAX_CLIENTS) {
    fprintf (stderr,
	     "num_clients must be greater than 0 but smaller than %d!\n",
	     MAX_CLIENTS);
    exit (-1);
  }
  ...
  while ((client_count < num_clients) &&
	 ...
	 && (client_tracker =
	     accept (tracker_socket, (struct sockaddr *) &tracker, &sckln))) {
    ...
    c.end_pos =
      (client_count ==
       num_clients - 1) ? num_bytes : c.start_pos + bytes_per_partition;
    ...
    assert (pthread_create (tids + client_count, NULL, handle_client, &c) ==
	    0);
    ++client_count;
  }
...
void *handle_client (void *args) {
  ...
  memset (buffer, 0, sizeof (buffer));
  sprintf (buffer, "You are client #%d. Sending (%d) bytes.",
	   c->client_number, bytes_to_send);
