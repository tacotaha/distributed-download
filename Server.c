...
int main(int argc, char *argv[]) {
  ...
  while ((client_count < num_clients) &&
    ...
    c.end_pos = (client_count == num_clients - 1)
                    ? num_bytes
                    : c.start_pos + bytes_per_partition;
...
void handle_client(Client *c) {
  ...
  memset(buffer, 0, sizeof(buffer));
  sprintf(buffer, "You are client #%d. Sending (%d) bytes.", c->client_number,
          bytes_to_send);
  assert(send(c->client_socket, buffer, BUF, 0) > 0);
  assert((fd = open(c->file_name, O_RDONLY)) != 1);

  while ((sent_bytes = sendfile(
              c->client_socket, fd, &offset,
              ((bytes_remaining - BUF) < 0) ? bytes_remaining : BUF)) > 0 &&
         (bytes_remaining > 0)) {
    bytes_remaining -= sent_bytes;
    printf("[+] %lf%c: ", 100.0 * (1 - (double)bytes_remaining / bytes_to_send),
           '%');
