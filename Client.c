...
int main (int argc, char *argv[]) {
  int client_socket, tracker_socket, inbound_sockets[MAX_CLIENTS],
    outbound_sockets[MAX_CLIENTS], outfds[MAX_CLIENTS], yes = 1, status = 0,
    port = PORT, i, tracker_port = port + 1, client_number, bytes_remaining,
    bytes_rcvd, num_peers = 0, client_index, thread_index = 0;
  ...
  for (int i = 0; i < num_peers; ++i)
	...
	status =
	  connect (inbound_sockets[i],
		   (struct sockaddr *) inbound_addrs + i,
  ...
  for (i = 0; i < num_peers; ++i) {
    ...
    assert (pthread_create
	    (tids + thread_index++, NULL, send_thread, thread_args + i) == 0);
    printf ("[+] Spawned outbound thread to client %d.\n", ga->client_index);
    assert (pthread_create
	    (tids + thread_index++, NULL, recv_thread, thread_args + i) == 0);
